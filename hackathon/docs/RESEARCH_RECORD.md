...
**Scope:** Completed work only. Planned follow-on experiments live in [FUTURE_WORK.md](FUTURE_WORK.md).  
...
- W≥3 scored higher than W=2 in the preliminary window read (r=0.690 vs r=0.600). Both at n=20, where a 0.09 gap is inside noise — this informed the choice, it did not establish it
...
Tested in early exploration. At λ=0.3, transition signal contributes meaningfully without overwhelming the binary outcome reward. At λ=1.0, the transition signal dominates and the model may optimize reasoning quality at the expense of answer correctness.
...
We tested:
- **GPT-4.1-mini (early reference):** r=0.600, 90.3% separation on 93 problems. Reference baseline.
- **Qwen2.5-7B-Instruct (production):** r=0.734, 95% separation on 20 problems. Point estimate above GPT-4.1-mini, but the 95% CIs overlap almost entirely ([0.43, 0.89] vs [0.45, 0.72]) — competitive, not demonstrably better.
...
**Why Qwen2.5-7B scores at least as well as GPT-4.1-mini:** Not obvious, and at n=20 the gap may not be real. Possibly better instruction following for the specific SCORE format, or better calibration on mathematical step evaluation. We report the empirical result without a definitive explanation, and note that the comparison is underpowered.
...
| Max steps | 400 | Compute budget; ~20% of GSM8K data |
...
| λ | 0.3 | Tuned qualitatively in early exploration |
| Window sizes | {2,3,5} P=(0.1,0.5,0.4) | Set from preliminary read |
...
## 4.1 Judge Validation (Pre-Training)
...
**Key finding:** Qwen2.5-7B is competitive with GPT-4.1-mini at n=20 (CIs overlap heavily; see results/judge_validation/summary.json). That is sufficient to justify it as the production judge and to remove the OpenAI API dependency. It is not sufficient to claim it is the better judge — that would need roughly n=80.

**Window size comparison (preliminary):**
...
Conclusion at the time: W≥3 is better, so the window probability distribution was updated to favor W=3. In hindsight both numbers come from n=20 sets and the 0.09 gap is not separable from noise. The distribution choice is defensible but under-evidenced.
...
**Story B is the stronger scientific claim.** The policy drift analysis proposed in [FUTURE_WORK.md](FUTURE_WORK.md) is specifically designed to distinguish between A and B.
...
3. Qwen2.5-7B is at least as good a judge as GPT-4.1-mini for mathematical reasoning (r=0.734 vs 0.600, overlapping CIs at n=20 vs n=93).
...
Evidence: r=0.734, 95% separation on 20 held-out problems — 95% CI [0.43, 0.89]. That interval contains GPT-4.1-mini's 0.600, so the comparison is underpowered. 100-problem validation is needed and is inference-only, no training required.
...
Validity: Tested qualitatively during early exploration. Not formally ablated at 400-step training scale. The optimal λ may differ with 7B student, more training steps, or different task.
...
# PART VI: Complete Results Reference
...
|------|-------|---|-----|---|--------|
| GSM8K | GPT-4.1-mini | 0.600 | 90.3% | 93 | Early reference |
...
| `judge_validation_strategyqa.py` | SQA judge test | `run_validation` |
| `judge_validation_math.py` | MATH judge test (failure) | Documents impossibility |
...
# StrategyQA TAPR  
python src/train_grpo.py --task strategyqa --mode tapr --lambda_val 0.3 \
  --output_dir ./runs/sqa_tapr
```

---

# PART VII: What You Must Own — Core Theory
