...
def main():
    ...
    # ── per-problem results ───────────────────────────────────────────────────
    # The summary above deliberately strips the per-problem `results` arrays to
    # stay readable. Write them to a sibling file instead of discarding them —
    # without these, no one (including us) can verify a reported number, run an
    # error analysis, or slice accuracy by problem difficulty.
    base, ext = os.path.splitext(args.output_file)
    per_problem_file = f"{base}_per_problem{ext}"
    per_problem = {
        "note": (
            "Per-problem eval outcomes. `id` indexes into the benchmark split in "
            "load order, so ids are comparable across models evaluated in the same "
            "run. Pair with the summary file of the same name for aggregates."
        ),
        "config": output["config"],
        "models": {
            model: {
                bench: v[bench].get("results", [])
                for bench in ("gsm8k", "math500", "strategyqa")
            }
            for model, v in all_results.items()
        },
    }
    with open(per_problem_file, "w") as f:
        json.dump(per_problem, f, indent=2)
    print(f"Per-problem results saved to: {per_problem_file}")
